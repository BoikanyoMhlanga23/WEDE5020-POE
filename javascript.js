...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm && contactForm.querySelector('input[name="name"]')) {
    ...
    const submitBtn = contactForm.querySelector('#submitBtn') || contactForm.querySelector('[type="submit"]');
    ...
    function validateForm() {
      ...
      if (!phoneInput.value.trim()) {
        ...
        errors.push('Phone number must be 10 digits and start with 0 (e.g. 0821234567).');
      }
    ...
    contactForm.addEventListener('submit', (e) => {
      ...
      // Show loading state (guard in case submit button isn't present)
      ...
      setTimeout(() => {
        ...
        // (adjust delay in milliseconds if you want a longer/shorter pause)
        setTimeout(() => {
          location.reload();
        }, 3000);
      }, 1500);
